Shader "CR/CleanOutline22"
{
    ...
    SubShader
    {
        ...
        Pass
        {
  ...
  #if UNITY_VERSION >= 202230 && UNITY_VERSION < 60000000  
			...
			half4 FragExtra(VaryingsExtra input) : SV_Target
			{
                ...
                float3 offset = float3((1.0 / screenWidth), (1.0 / screenHeight), 0.0) *
                ...
                sobelDepth = ShapeOutlineSignal(sobelDepth, _DepthThreshold,
                                                _OutlineDepthMultiplier, _OutlineDepthBias);
                ...
                if (_EnableNormalOutline == 1)
                {
                    ...
                    sobelNormal = ShapeOutlineSignal(sobelNormal, _NormalThreshold,
                                                     _OutlineNormalMultiplier, _OutlineNormalBias);
                }
                float isFarAway = step(0.9999999, depth01);
               
                sobelNormal = saturate(abs(sobelNormal)) * (1 - isFarAway);
					
                float outlineStrength = max(sobelNormal, sobelDepth) * distanceFade;
                outlineStrength = saturate(outlineStrength);

                float3 colorCombined = lerp(baseColor.rgb, _OutlineColor.rgb,
                                            outlineStrength * _OutlineColor.a);
